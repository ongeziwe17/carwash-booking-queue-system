...
Responses round distance/price to their public scales and scores/contributions to six decimals. BEST_OVERALL totals are the exact sum of the four displayed contributions. Deterministic customer-safe templates explain the applied metric and explicitly describe unavailable queue/total data. Recommendations are non-reserving point-in-time reads; booking creation may reject a stale result and always revalidates branch, offering, local time, capacity, ownership, and vehicle conflicts under the write boundary.
