...
The application is a Spring Boot modular monolith organized by the identity, access, vehicle, catalog, booking, queue, notification, reporting, marketplace, and discovery capabilities. Repository contracts and implementations belong to their owning capabilities, while narrow immutable application queries expose authorization, operational, reporting, Marketplace, Catalog, and discovery reads and one shared coordinator protects the in-memory repositories. Identity owns roles, permissions, and the credential contract; Access implements authentication, BCrypt/JWT infrastructure, and authorization. The current domain covers users, roles, vehicles, reusable services, branch service offerings, branch-scoped bookings and queues, in-app notifications, scoped daily reports, car wash businesses, physical branches, weekly branch schedules, temporary closures, and nearby branch projections. PostgreSQL persistence, tenant isolation, branch-aware availability, payments, and external notification delivery remain future work.
...
`BookingSlotPolicyService` remains the scheduling decision component for creation, rescheduling, generic offering changes, and AVAIL-001. Operational booking calls use branch-offering duration and partition non-cancelled exact-start capacity/conflicts by branch. They still enforce the transitional configured global opening/closing window and interval grid; OPS-001 does not enforce Marketplace hours. AVAIL-001 retains its legacy global-service/global-capacity read model until AVAIL-002.
...
Availability does not mutate or reserve domain state. Booking creation remains the authoritative branch-scoped write operation and rechecks capacity plus customer/vehicle rules under the coordinator write lock. AVAIL-001 does not yet select a branch/offering or consume Marketplace hours; there is still no staff, bay, holiday, recommendation, offering-capacity calculation, or temporary-hold aggregate.
...
Queue entry eligibility, active uniqueness, branch-partitioned ordering/recalculation/cumulative waits/call-next, focused booking rescheduling, legacy single-location availability, Marketplace scheduling, and booking/queue lifecycle synchronization are implemented. Canonical queue scope integrity is distinct from operational activity: creation, call, and start require both, while completion of already `IN_PROGRESS` work requires canonical identity/referential integrity but remains possible after parent deactivation. Queue creation and call keep the booking `CONFIRMED`; start and completion synchronize both aggregates. Valid booking cancellation removes `WAITING`/`CALLED` queue work and rebalances only the booking branch, while in-service cancellation is rejected. Generic booking updates and rescheduling are blocked whenever an active queue entry exists. Operational creation/rescheduling/offering changes reuse the same transitional operating-window/slot policy with branch partitioning; cancellation and rescheduling share the deployment-configurable booking-change cutoff. Marketplace hours remain separate and do not retrofit booking or AVAIL-001 in OPS-001.
