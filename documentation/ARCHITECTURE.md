...
| `audit` | Immutable audit model/action catalogue, append/query-only repository port, metadata policy, transaction coordinator, in-memory/PostgreSQL adapters, bounded query service, and read-only HTTP API |
...
Application services depend on `shared.application.DataTransactionOperations`, never on a storage implementation. Tenant/subject authorization for a mutation is evaluated after its resource lock and inside the same authoritative write boundary as the mutation. Sensitive operations enter an outer audit write boundary; where success scope depends on a canonical resource, a bounded deferred command supplier acquires the mutation's deterministic lock set and performs the scoped canonical read inside that boundary. The mandatory SUCCESS audit insert then occurs before the protected mutation and joins the same authoritative transaction. The protected mutation reuses the held transaction-scoped locks and retains its guarded tenant/subject repository predicates. If the insert fails, the mutation never begins. If the protected operation fails, PostgreSQL rolls back the provisional success row and business writes; the in-memory adapter invokes registered audit rollback plus existing aggregate compensation. Audit then appends one DENIED/FAILURE row through `isolatedWrite` (`REQUIRES_NEW` on PostgreSQL, the shared fair write lock in memory) without masking the original safe error if that append also fails. This is not an after-commit audit callback.
...
`READ_COMMITTED`; independently optimizable count and page CTEs query the authorized table scope directly, and the
count is left-joined to the page so an exhausted cursor still returns the full unread total. The in-memory adapter
derives both values while holding its existing repository monitor. Neither design changes isolation for unrelated
reads or relies on a JVM-wide/distributed lock.
...
| Mandatory success audit | Same authoritative REQUIRED write | append-only insert before protected work | no additional logical mutation lock |
| Denied/failed audit | isolated write after rollback | append-only insert; no unscoped enrichment | `REQUIRES_NEW` / shared write lock |
...
The default lightweight profile remains in memory; durable audit retention requires `postgres`. Elevated operational/Marketplace access is tenant-scoped, notifications remain in-app only, and reporting remains a basic tenant-scoped snapshot. Nearby distance is straight-line rather than driving distance and has no traffic, route, geocoding, external maps, cache, or geospatial index. Availability is a non-reserving snapshot. SIEM export, archive storage, automated audit purge, compliance certification, payments/refunds, concurrent bay/staff allocation, managed database provisioning, backups, replicas, messaging, and distributed transactions remain intentionally absent.
