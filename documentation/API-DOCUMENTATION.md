...
- Current controller/OpenAPI operation count: **53**
...
|---|---|
| `CUSTOMER` | Own profile, vehicles, bookings, queue-entry reads, and notifications; service-catalogue, availability, and discoverable-branch reads. |
| `STAFF` | Own profile/notifications plus global vehicle, booking, and queue operational access; service-catalogue, availability, and discoverable-branch reads. |
...
Business and branch records begin active. Business deactivation leaves branch records unchanged but makes them ineffective and undiscoverable; reactivation restores discovery only for active branches that still opt in. This foundation does not provide tenant ownership enforcement, branch hours, branch service offerings, distance/radius calculations, branch-specific booking/queue operations, or branch-aware availability.
