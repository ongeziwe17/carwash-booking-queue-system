...
public class DailySummaryReportService {
    ...
    public DailySummaryReportService(BookingRepository bookingRepository, QueueEntryRepository queueEntryRepository) {
    ...
    public DailySummaryReportResponse generateDailySummary(LocalDate reportDate) {
        ...
        List<Booking> bookingsForDate = bookingRepository.findAll().stream()
                ...
                .filter(booking -> booking.getScheduledDateTime().toLocalDate().equals(reportDate))
                .toList();
    ...
    private long countQueueEntriesByStatus(List<QueueEntry> queueEntries, QueueStatus status) {
        return queueEntries.stream()
                .filter(queueEntry -> queueEntry.getQueueStatus() == status)
                .count();
    }
}
