...
public class PostgresCarWashBranchRepository implements CarWashBranchRepository {
    ...
    @Override public boolean updateForBusiness(CarWashBranch v,String businessId){
        Optional<BranchJpaEntity> found=repository.findByIdAndBusinessId(v.getBranchId(),businessId);
        return found.isPresent()&&guardedUpdate(v,found.get().version,businessId);
    }
    @Override public boolean updateForAdministrator(CarWashBranch v){
        Optional<BranchJpaEntity> found=repository.findById(v.getBranchId());
    ...
    private boolean guardedUpdate(CarWashBranch v,Long version,String businessId){
        int rows=businessId==null
                ? repository.updateAdministratorScoped(
                        v.getBranchId(),v.getBranchName(),v.getAddressLine1(),v.getAddressLine2(),v.getCity(),
                        v.getProvince(),v.getPostalCode(),v.getCountryCode(),v.getLatitude(),v.getLongitude(),
                        v.getTimezone(),v.getStatus().name(),v.isPublicDiscoveryEnabled(),
                        PersistenceSupport.databaseTime(v.getUpdatedAt()),
                        PersistenceSupport.nanoRemainder(v.getUpdatedAt()),version)
                : repository.updateBusinessScoped(
                        v.getBranchId(),businessId,v.getBranchName(),v.getAddressLine1(),v.getAddressLine2(),
                        v.getCity(),v.getProvince(),v.getPostalCode(),v.getCountryCode(),v.getLatitude(),
                        v.getLongitude(),v.getTimezone(),v.getStatus().name(),v.isPublicDiscoveryEnabled(),
                        PersistenceSupport.databaseTime(v.getUpdatedAt()),
                        PersistenceSupport.nanoRemainder(v.getUpdatedAt()),version);
        return rows==1;
    }
