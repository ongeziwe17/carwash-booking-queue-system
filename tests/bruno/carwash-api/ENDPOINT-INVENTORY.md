...
Inventory source: the controllers and OpenAPI quality gate on `staging` at `541352c`. Generated `/v3/api-docs` is authoritative when wording differs.
...
| `POST` | `/api/queue-entries/{id}/call-next` | Bearer | STAFF, BUSINESS_OWNER, PLATFORM_ADMIN | Operational access | `—` | 200 | 400,401,403,404 | Marks this supplied ID WAITING -> CALLED; it does not server-select the next entry. |
