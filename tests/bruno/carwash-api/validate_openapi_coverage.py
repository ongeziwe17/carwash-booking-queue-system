...
def normalize_bruno_path(url: str) -> str:
    ...
    if path.startswith("/api/admin/users/") and path.endswith("/tenant-membership"):
        return re.sub(r"/\{\{[^}]+\}\}/tenant-membership$",
                      "/{userId}/tenant-membership", path)
    if path.startswith("/api/notifications/user/"):
        return re.sub(r"^/api/notifications/user/\{\{[^}]+\}\}",
                      "/api/notifications/user/{userId}", path)
    if path.startswith("/api/notifications/"):
        return re.sub(r"^/api/notifications/\{\{[^}]+\}\}",
                      "/api/notifications/{notificationId}", path)
    ...
    if path.startswith("/api/marketplace/closures/") and path.endswith("/cancel"):
        return re.sub(r"^/api/marketplace/closures/\{\{[^}]+\}\}",
                      "/api/marketplace/closures/{closureId}", path)
    if path.startswith("/api/marketplace/offerings/"):
        return re.sub(r"^/api/marketplace/offerings/\{\{[^}]+\}\}",
                      "/api/marketplace/offerings/{offeringId}", path)
